...
def construct_waltz():
    minuetList = []
    for i in range(0,len(minuet_table)):
        minuetList.append(minuet_table[i][roll_dice(2)])
    trioList = []
    for i in range(0,len(trio_table)):
        trioList.append(trio_table[i][roll_dice()])
    waveList = []
    for i in minuetList:
        waveList.append(simpleaudio.WaveObject.from_wave_file(minuet_filename(i)))
    for i in trioList:
        waveList.append(simpleaudio.WaveObject.from_wave_file(trio_filename(i)))
